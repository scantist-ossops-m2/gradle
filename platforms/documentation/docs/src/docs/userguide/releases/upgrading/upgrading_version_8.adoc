...
In regular Groovy, this would be just a method call: `propertyName(value)`.
...
It is now deprecated and will be removed in Gradle 10.0.
Use explicit assignment `propertyName = value` syntax or a method call instead.
Note that in some places there are explicit methods that are named exactly like property.
While they can be used like `propertyName value`, it is recommended to use the `propertyName(value)` syntax for consistency.

[[deprecated_configuration_cache_options]]
