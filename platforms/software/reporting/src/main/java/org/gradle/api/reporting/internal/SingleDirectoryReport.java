...
public abstract class SingleDirectoryReport extends SimpleReport implements DirectoryReport {
    ...
    @Override
    public Provider<File> getEntryPoint() {
        ...
        if (relativeEntryPath == null) {
            return getOutputLocation().getLocationOnly().map(Directory::getAsFile);
        } else {
            return getOutputLocation().getLocationOnly().map(dir -> dir.file(relativeEntryPath).getAsFile());
        }
    }
}
